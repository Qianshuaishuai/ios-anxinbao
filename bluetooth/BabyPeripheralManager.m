    ...
    return ^BabyPeripheralManager *() {
        ...
        if ([self canStartAdvertising]) {
            ...
            if (PERIPHERAL_MANAGER_INIT_WAIT_TIMES > 5) {
                BabyLog(ASLocalizedString(@">>>error： 第%d次等待peripheralManager打开任然失败，请检查蓝牙设备是否可用"),PERIPHERAL_MANAGER_INIT_WAIT_TIMES);
            }
            dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, 3.0 * NSEC_PER_SEC);
            dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                self.startAdvertising();
            });
            BabyLog(ASLocalizedString(@">>> 第%d次等待peripheralManager打开"),PERIPHERAL_MANAGER_INIT_WAIT_TIMES);
        }
