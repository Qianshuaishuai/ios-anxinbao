...
- (void)start:(CBPeripheral *)cachedPeripheral {
    ...
    if (CENTRAL_MANAGER_INIT_WAIT_TIMES >= KBABY_CENTRAL_MANAGER_INIT_WAIT_TIMES ) {
        BabyLog(ASLocalizedString(@">>> 第%d次等待CBCentralManager 打开任然失败，请检查你蓝牙使用权限或检查设备问题。"),CENTRAL_MANAGER_INIT_WAIT_TIMES);
        return;
        //[NSException raise:ASLocalizedString(@"CBCentralManager打开异常")format:ASLocalizedString(@"尝试等待打开CBCentralManager5次，但任未能打开")];
    }
    ...
    BabyLog(ASLocalizedString(@">>> 第%d次等待CBCentralManager打开"),CENTRAL_MANAGER_INIT_WAIT_TIMES);
}
...
- (void)validateProcess {
    ...
    if (!babyCentralManager->needDiscoverDescriptorsForCharacteristic) {
        if (babyCentralManager->needReadValueForDescriptors) {
            [faildReason addObject:ASLocalizedString(@"未执行discoverDescriptorsForCharacteristic()不能执行readValueForDescriptors()")];
        }
    }
    ...
    if (!babyCentralManager->needDiscoverCharacteristics) {
        if (babyCentralManager->needReadValueForCharacteristic||babyCentralManager->needDiscoverDescriptorsForCharacteristic) {
            [faildReason addObject:ASLocalizedString(@"未执行discoverCharacteristics()不能执行readValueForCharacteristic()或discoverDescriptorsForCharacteristic()")];
        }
    }
    ...
    if (!babyCentralManager->needDiscoverServices) {
        if (babyCentralManager->needDiscoverCharacteristics||babyCentralManager->needDiscoverDescriptorsForCharacteristic ||babyCentralManager->needReadValueForCharacteristic ||babyCentralManager->needReadValueForDescriptors) {
             [faildReason addObject:ASLocalizedString(@"未执行discoverServices()不能执行discoverCharacteristics()、readValueForCharacteristic()、discoverDescriptorsForCharacteristic()、readValueForDescriptors()")];
        }
    ...
    if(!babyCentralManager->needConnectPeripheral) {
        if (babyCentralManager->needDiscoverServices) {
             [faildReason addObject:ASLocalizedString(@"未执行connectToPeripherals()不能执行discoverServices()")];
        }
    }
    ...
    if (!babyCentralManager->needScanForPeripherals) {
        ...
        if (!peripheral) {
            [faildReason addObject:ASLocalizedString(@"若不执行scanForPeripherals()方法，则必须执行connectToPeripheral方法并且需要传入参数(CBPeripheral *)peripheral")];
        }
    }
...
- (BabyBluetooth *(^)(CBPeripheral *peripheral,CBCharacteristic *characteristic)) characteristicDetails {
    ...
    return ^(CBPeripheral *peripheral,CBCharacteristic *characteristic) {
        ...
        if (peripheral.state == CBPeripheralStateConnected) {
            ...
            BabyLog(ASLocalizedString(@"!!!设备当前处于非连接状态"));
        }
